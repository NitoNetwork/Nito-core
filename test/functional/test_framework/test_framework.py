...
class NitoTestMetaClass(type):
    """Metaclass for NitoTestFramework.

    Ensures that any attempt to register a subclass of `NitoTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'NitoTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("NitoTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("NitoTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class NitoTestFramework(metaclass=NitoTestMetaClass):
    """Base class for a nito test script.

    Individual nito test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave nitods and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop nitods after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use nito-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "nitod": ("nitod", "BITCOIND"),
            "nito-cli": ("nitocli", "BITCOINCLI"),
            "nito-util": ("nitoutil", "BITCOINUTIL"),
            "nito-wallet": ("nitowallet", "BITCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: nitods were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("NitoRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'nitod', self.options.nitod) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'nito-cli', self.options.nitocli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a nitod test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple nitod test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the nitod
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as nitod's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    nitod=self.options.nitod,
                    nito_cli=self.options.nitocli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in nito.conf
    ...
    def skip_if_no_nitod_tracepoints(self):
        """Skip the running test if nitod has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("nitod has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_nitod_zmq(self):
        """Skip the running test if nitod has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("nitod has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if nito-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("nito-wallet has not been compiled")

    def skip_if_no_nito_util(self):
        """Skip the running test if nito-util has not been compiled."""
        if not self.is_nito_util_compiled():
            raise SkipTest("nito-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if nito-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("nito-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether nito-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether nito-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_nito_util_compiled(self):
        """Checks whether nito-util was compiled."""
