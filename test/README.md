This directory contains integration tests that test nitod and its
...
nitod and nito-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (nito-util, nito-tx, ...).
...
Before tests can be run locally, Nito Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the nitod nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another nitod
process running on the system (perhaps from a previous test which hasn't successfully
killed all its nitod nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other nitod processes are running.

On linux, the test framework will warn if there is another
nitod process running when the tests are started.

If there are zombie nitod processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
nitod processes running on the system, so should not be used if any non-test
nitod processes are being run.**
...
sure nitod processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and nitod `debug.log`s will all be dumped
...
`test_framework.log` and nitod `debug.log`s can be combined into a single
...
call methods that interact with the nitod nodes-under-test.

If further introspection of the nitod instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/nitod.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/nitod <pid>
```
