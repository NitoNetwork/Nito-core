...
#      define nito_builtin_bswap16(x) __builtin_bswap16(x)
#    endif
#    if __has_builtin(__builtin_bswap32)
#      define nito_builtin_bswap32(x) __builtin_bswap32(x)
#    endif
#    if __has_builtin(__builtin_bswap64)
#      define nito_builtin_bswap64(x) __builtin_bswap64(x)
#    endif
#  elif defined(_MSC_VER)
#      define nito_builtin_bswap16(x) _byteswap_ushort(x)
#      define nito_builtin_bswap32(x) _byteswap_ulong(x)
#      define nito_builtin_bswap64(x) _byteswap_uint64(x)
#  endif
#endif
...
{
#ifdef nito_builtin_bswap16
    return nito_builtin_bswap16(x);
#else
    return (x >> 8) | (x << 8);
#endif
}
...
{
#ifdef nito_builtin_bswap32
    return nito_builtin_bswap32(x);
#else
...
{
#ifdef nito_builtin_bswap64
    return nito_builtin_bswap64(x);
#else
