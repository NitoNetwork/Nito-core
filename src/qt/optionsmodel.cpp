...
/** Call node.updateRwSetting() with Nito 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Nito 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/nito/nito/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because nito 22.x will not try to read these.
...
//! Convert enabled/size values to nito -prune setting.
...
//! Get pruning enabled value to show in GUI from nito -prune setting.
...
//! Get pruning size value to show in GUI from nito -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayNitoUnit")) {
        settings.setValue("DisplayNitoUnit", QVariant::fromValue(NitoUnit::BTC));
    }
    QVariant unit = settings.value("DisplayNitoUnit");
    if (unit.canConvert<NitoUnit>()) {
        m_display_nito_unit = unit.value<NitoUnit>();
    } else {
        m_display_nito_unit = NitoUnit::BTC;
        settings.setValue("DisplayNitoUnit", QVariant::fromValue(m_display_nito_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if nito.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_nito_unit);
...
{
    if (new_unit.isNull() || new_unit.value<NitoUnit>() == m_display_nito_unit) return;
    m_display_nito_unit = new_unit.value<NitoUnit>();
    QSettings settings;
    settings.setValue("DisplayNitoUnit", QVariant::fromValue(m_display_nito_unit));
    Q_EMIT displayUnitChanged(m_display_nito_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/nito/nito/pull/8273
    ...
    // (https://github.com/nito-core/gui/issues/567).
