...
#include <qt/nitogui.h>

#include <qt/nitounits.h>
...
const std::string NitoGUI::DEFAULT_UIPLATFORM =
...
NitoGUI::NitoGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &NitoGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &NitoGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &NitoGUI::showModalOverlay);
...
void NitoGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Nito address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and nito: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &NitoGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &NitoGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &NitoGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Nito addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Nito addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Nito Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Nito Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a nito: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Nito command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &NitoGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &NitoGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &NitoGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &NitoGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &NitoGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &NitoGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &NitoGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &NitoGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &NitoGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &NitoGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &NitoGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &NitoGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &NitoGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &NitoGUI::showDebugWindow);
}

void NitoGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void NitoGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &NitoGUI::setCurrentWalletBySelectorIndex);
...
void NitoGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &NitoGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &NitoGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &NitoGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &NitoGUI::showProgress);
...
#ifdef ENABLE_WALLET
void NitoGUI::enableHistoryAction(bool privacy)
{
...
void NitoGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &NitoGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &NitoGUI::removeWallet);
...
WalletController* NitoGUI::getWalletController()
{
    return m_wallet_controller;
}

void NitoGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &NitoGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &NitoGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &NitoGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &NitoGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &NitoGUI::incomingTransaction);
    connect(this, &NitoGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void NitoGUI::removeWallet(WalletModel* walletModel)
{
...
void NitoGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void NitoGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void NitoGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void NitoGUI::setWalletActionsEnabled(bool enabled)
{
...
void NitoGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &NitoGUI::toggleHidden);
...
void NitoGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void NitoGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void NitoGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void NitoGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void NitoGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void NitoGUI::openClicked()
{
...
void NitoGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void NitoGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void NitoGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void NitoGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void NitoGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void NitoGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void NitoGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void NitoGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Nito network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void NitoGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void NitoGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void NitoGUI::updateHeadersSyncProgressLabel()
{
...
void NitoGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void NitoGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &NitoGUI::quitRequested);
...
void NitoGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void NitoGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &NitoGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void NitoGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void NitoGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &NitoGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &NitoGUI::show);
                e->ignore();
            }
        }
    }
...
void NitoGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void NitoGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void NitoGUI::incomingTransaction(const QString& date, NitoUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(NitoUnits::formatWithUnit(unit, amount, true));
...
void NitoGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void NitoGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool NitoGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool NitoGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void NitoGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void NitoGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void NitoGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void NitoGUI::updateWindowTitle()
{
...
void NitoGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void NitoGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void NitoGUI::detectShutdown()
{
...
void NitoGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void NitoGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(NitoGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = NitoGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void NitoGUI::subscribeToCoreSignals()
{
...
void NitoGUI::unsubscribeFromCoreSignals()
{
...
bool NitoGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<NitoUnit> units = NitoUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const NitoUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, NitoUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const NitoUnit u : NitoUnits::availableUnits()) {
        menu->addAction(NitoUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(NitoUnit newUnits)
{
    setText(NitoUnits::longName(newUnits));
}
