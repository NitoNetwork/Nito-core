...
const QString BITCOIN_IPC_PREFIX("nito:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // nito: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click nito: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start nito: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling nito: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("nito://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'nito://' is not a valid URI. Use 'nito:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // nito: URI
    {
        QUrlQuery uri((QUrl(s)));
        // normal URI
        {
            SendCoinsRecipient recipient;
            if (GUIUtil::parseNitoURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Nito address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
