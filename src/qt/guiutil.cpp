...
#include <qt/nitoaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Nito address (e.g. nito1qsd3slhjrfpks6twytesgj04vxecg5du0a83xdj)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new NitoAddressEntryValidator(parent));
    widget->setCheckValidator(new NitoAddressCheckValidator(parent));
}
...
bool parseNitoURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no nito: URI
    if(!uri.isValid() || uri.scheme() != QString("nito"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!NitoUnits::parse(NitoUnit::BTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseNitoURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseNitoURI(uriInstance, out);
}

QString formatNitoURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("nito:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(NitoUnits::format(NitoUnit::BTC, info.amount, false, NitoUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openNitoConf()
{
    ...
    /* Open nito.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Nito.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Nito (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Nito (%s).lnk", ChainTypeToString(chain)));
}
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "nito.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("nito-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a nito.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Nito\n";
        else
            optionFile << strprintf("Name=Nito (%s)\n", ChainTypeToString(chain));
