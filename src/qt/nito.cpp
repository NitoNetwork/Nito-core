...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<NitoUnit>("NitoUnit");
#else
    qRegisterMetaType<NitoUnit>("NitoUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. nito_de.qm (shortcut "de" needs to be defined in nito.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. nito_de_DE.qm (shortcut "de_DE" needs to be defined in nito.qrc)
...
static const char* qt_argv = "nito-qt";

NitoApplication::NitoApplication()
...
void NitoApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the NitoApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", NitoGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void NitoApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool NitoApplication::createOptionsModel(bool resetSettings)
{
...
void NitoApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new NitoGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &NitoGUI::quitRequested, this, &NitoApplication::requestShutdown);
...
void NitoApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void NitoApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool NitoApplication::baseInitialize()
{
    return node().baseInitialize();
}

void NitoApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &NitoApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &NitoApplication::handleRunawayException);
    connect(this, &NitoApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &NitoApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void NitoApplication::parameterSetup()
{
...
void NitoApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void NitoApplication::requestInitialize()
{
...
void NitoApplication::requestShutdown()
{
...
void NitoApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // nito: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NitoGUI::handlePaymentRequest);
            connect(window, &NitoGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void NitoApplication::handleRunawayException(const QString &message)
{
...
void NitoApplication::handleNonFatalException(const QString& message)
{
...
WId NitoApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool NitoApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", NitoGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(nito);
    Q_INIT_RESOURCE(nito_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see nito-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see nito-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse nito.conf, determine network, switch to network specific
    ...
    // nito: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
