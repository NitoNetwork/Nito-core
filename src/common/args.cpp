...
const char * const BITCOIN_CONF_FILENAME = "nito.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //nito-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Nito
    // macOS: ~/Library/Application Support/Nito
    // Unix-like: ~/.nito
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nito";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Nito";
#else
    // Unix-like
    return pathRet / ".nito";
#endif
