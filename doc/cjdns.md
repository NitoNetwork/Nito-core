# CJDNS support in Nito Core

It is possible to run Nito Core over CJDNS, an encrypted IPv6 network that
...
done outside of Nito Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Nito network and individual nodes.
...
before it will work with your Nito Core node. This is described in steps
...
## Run Nito Core with CJDNS

Once you are connected to the CJDNS network, the following Nito Core
...
When enabled, this option tells Nito Core that it is running in an
...
IPv6 local network. This helps Nito Core perform better address management:
...
CJDNS support was added to Nito Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `nito-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/nito/nito/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Nito Core

There are several ways to see your CJDNS address in Nito Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `nito-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `nito-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `nito-cli help getnodeaddresses` for details.
