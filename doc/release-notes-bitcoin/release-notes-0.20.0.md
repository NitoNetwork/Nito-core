...
Nito Core version 0.20.0 is now available from:

  <https://nitocore.org/bin/nito-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Nito Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Nito Core version 0.18.0.
...
- Testing or debugging of implementations of the Nito P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Nito Core. Nito Core logs debug messages
...
- OpenSSL is no longer used by Nito Core.  (#17265)

- BIP70 support has been fully removed from Nito Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/nito/nito/issues/16599),
  [PR #16702](https://github.com/nito/nito/pull/16702), and the
  `nitod help` for more information.  This option is experimental and
...
- All custom settings configured when Nito Core starts are now
...
- The "Start Nito Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://nitocore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Nito Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Nito Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/nito-core/HWI) for
  signing.  Future versions of Nito Core should support a GUI option
...
- Nito Core's automatically-generated source code documentation is
  now available at https://doxygen.nitocore.org.  (#17596)
...
- The `nito-cli` utility used with the `-getinfo` parameter now
...
- The `nito-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `nitostrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_nito-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_nito` binary (hebasto)
...
- #18702 Fix ASLR for nito-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate nito schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest nito.conf (sdaftuar)
...
- #18022 Fix appveyor `test_nito` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (nito-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone nito-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Nito Core files (hebasto)
...
- #17119 Fix broken nito-cli examples (andrewtoth)
...
- #17503 Remove bitness from nito-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.nitocore.org (laanwj)
...
- #17648 Rename wallet-tool references to nito-wallet (hel-o)
...
- #16115 On nitod startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from nito-cli -getinfo (malevolent)
