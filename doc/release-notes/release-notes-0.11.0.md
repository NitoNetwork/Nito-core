Nito Core version 0.11.0 is now available from:

  <https://nito.org/bin/nito-core-0.11.0/>
...
For example, add the following to `nito.conf`:
...
related to the blockchain in the nito system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Nito Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Nito Core will 
...
For an example script see [nito-submittx](https://github.com/laanwj/nito-submittx).
...
- #5334 `cf87536` libnitoconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new nito-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only NitoAmountField
- #5753 `0f44672` Add nito logo to about screen
...
- #5809 `46bfbe7` Add nito-cli man page
...
- #5849 `41259ca` contrib/init/nitod.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Nito-Qt.app to Nito-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for nito-tx)
