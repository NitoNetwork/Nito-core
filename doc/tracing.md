# User-space, Statically Defined Tracing (USDT) for Nito Core

Nito Core includes statically defined tracepoints to allow for more
...
**Note**: Nito Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Nito Core
...
Multiple tools can list the available tracepoints in a `nitod` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Nito Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/nitod
stap net        outbound_message 0x0000000000107c05 /src/nitod
stap validation block_connected  0x00000000002fb10c /src/nitod
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Nito Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/nitod | grep NT_STAPSDT -A 4 -B 2
