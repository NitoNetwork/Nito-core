...
dnl Output: If qt version is auto, set nito_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "$nito_qt_want_version" = "auto" && test "$nito_qt_force" != "yes"; then
    if test "$nito_enable_qt" != "no"; then
      AC_MSG_WARN([$1; nito-qt frontend will not be built])
    fi
    nito_enable_qt=no
    nito_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "$nito_enable_qt" != "no" && test "$nito_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build nito-qt GUI (default=auto)])],
    [
     nito_qt_want_version=$withval
     if test "$nito_qt_want_version" = "yes"; then
       nito_qt_force=yes
       nito_qt_want_version=auto
     fi
    ],
    [nito_qt_want_version=auto])
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); nito_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl Outputs: nito_enable_qt, nito_enable_qt_dbus, nito_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "$nito_cv_static_qt" = "yes"; then
  ...
  if test $nito_enable_qt = "yes"; then
    AC_MSG_RESULT([$nito_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$nito_enable_qt])
  fi
...
dnl Output: nito_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, nito_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [nito_cv_static_qt=yes],
      [nito_cv_static_qt=no])
    ])
])
