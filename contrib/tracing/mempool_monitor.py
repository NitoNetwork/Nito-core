...
""" Example logging Nito Core mempool events using the mempool:added,
...
def main(nitod_path):
    nitod_with_usdts = USDT(path=str(nitod_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    nitod_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    nitod_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    nitod_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    nitod_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[nitod_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/nitod")
        exit(1)
