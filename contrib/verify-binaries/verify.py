...
"""Script for verifying Nito Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from nitocore.org and nito.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/nito-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://nitocore.org"
HOST2 = "https://nito.org"
VERSIONPREFIX = "nito-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Nito community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"nito_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by nitocore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the nito release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes nito.org lags behind nitocore.org.)')
    )
